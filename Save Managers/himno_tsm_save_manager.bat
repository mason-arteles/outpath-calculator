...
SET CURR_PATH="%APPDATA%/../LocalLow/Breakfast Studio/Himno The Silent Melody"
SET ARGS=%*
SET DO_LOOP="FALSE"
SET PREFIX=%0
...
CD Backups 2> NUL && DIR ".currentFile.txt" > NUL 2> NUL && GOTO :ScriptStart
    MKDIR %CURR_PATH%
    CD %CURR_PATH%
    COPY "..\SaveFile1.es3" "default.es3" || COPY "..\SaveFile2.es3" "default.es3" || COPY "..\SaveFile3.es3" "default.es3" || GOTO :FileNotFound
    ECHO default > ".currentFile.txt"

GOTO :ScriptStart

:FileNotFound
    ECHO "No current save files exist. Due to the design of the persistence of the script, it cannot launch until there is an existing save file."
GOTO :END_FILE
...
:CASE_load
    ...
    GOTO :CASE_LOAD_VALID_FILENAME %~1
        ...
        :GET_LOAD_FILE_SLOT
            SET /P "FILE_SLOT=Which slot do you want to write over? > "
            IF ("!FILE_SLOT!") == ("cancel") GOTO :END_CASE
            SET /A "FILE_SLOT=%FILE_SLOT%"
            IF !FILE_SLOT! GEQ 1 IF !FILE_SLOT! LEQ 3 GOTO :CASE_LOAD_HAS_CURR_SAVE_FILE
            ECHO "Invalid save slot, slots must be a number 1-3"
        GOTO :GET_LOAD_FILE_SLOT

        :CASE_LOAD_HAS_CURR_SAVE_FILE
        REM echo %CURR_SAVE_FILE%
        DIR "%CURR_SAVE_FILE%.es3" > NUL && (
            TYPE "%CURR_SAVE_FILE%.es3" > "..\SaveFile!FILE_SLOT!.es3"
        ) || (
...
:CASE_save
    ...
    :GET_SAVE_FILE_SLOT
        SET /P "FILE_SLOT=Which slot do you want to save from?"
        IF ("!FILE_SLOT!") == ("cancel") GOTO :END_CASE
        SET /A "FILE_SLOT=%FILE_SLOT%"
        IF %FILE_SLOT% GEQ 1 IF %FILE_SLOT% LEQ 3 GOTO :SAVE_LOOP_2_START
        ECHO "Invalid save slot, slots must be a number 1-3"
    GOTO :GET_SAVE_FILE_SLOT
    ...
    FOR /F "tokens=1" %%A IN ("!ARGS!") DO (
        ...
        ( TYPE "..\SaveFile!FILE_SLOT!.es3") > "%%A.es3"
    )
    CALL :NEXT_ARG
